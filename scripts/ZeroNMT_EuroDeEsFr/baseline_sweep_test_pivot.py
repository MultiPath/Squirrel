...
run = "scripts/ZeroNMT_EuroDeEsFr/baseline_europarl_test_pivot.sh"
slurm_options = "sbatch -J {} --partition={} --mem {}GB --gres=gpu:{} -c {} -C volta --time=00:15:00 --output={}/".format(
                'baselines', 'learnfair', 128, 2, 48, '/checkpoint/jgu/space/slurm')
    ...
    for src in lans:
        for trg in lans:
            for b in [1, 4]:
                ...
                slurm_options_ = slurm_options + (src + '-' + trg + '-' + str(b) + '-' + a.split()[0] + '.pivot_%j.out')
