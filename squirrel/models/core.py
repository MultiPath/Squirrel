...
from squirrel.utils import INF, TINY, colored_seq
...
def positional_encodings_like(x, t=None):  # hope to be differentiable
    ...
    # channels
    channels = torch.arange(0, x.size(-1), 2).float() / x.size(
        -1)  # 0 2 4 6 ... (256)
    if x.is_cuda:
    ...
    encodings = positions.unsqueeze(-1) @ channels.unsqueeze(
        0)  # batch x target_len x 256
    encodings = torch.cat([
        torch.sin(encodings).unsqueeze(-1),
        torch.cos(encodings).unsqueeze(-1)
    ], -1)
    encodings = encodings.contiguous().view(*encodings.size()[:-2],
                                            -1)  # batch x target_len x 512
...
def with_mask(targets, out, input_mask=None):
...
def sample_gumbel(tensor, eps=1e-20):
    return tensor.new_zeros(tensor.size()).uniform_().add_(
        TINY).log_().neg_().add_(TINY).log_().neg_()
...
def gumbel_sinkhorn(log_alpha,
    ...
    log_alpha_w_noise = log_alpha_w_noise.view(n_samples, batch_size, n,
                                               n).contiguous().transpose(0, 1)
...
def cross_entropy_with_smooth(outputs,
                              targets,
                              label_smooth=0.1,
                              reweight=None):
    ...
    if reweight is None:
        return F.nll_loss(logits, targets) * (
            1 - label_smooth) - logits.mean() * label_smooth
    else:

        nll_loss = (
            F.nll_loss(logits, targets, reduction='none') * reweight).mean()
        return nll_loss * (1 - label_smooth) - logits.mean() * label_smooth


def cross_entropy_matrix(outputs,
                         targets,
                         masks,
                         label_smooth=0.0,
                         sentence_wise=True):
    ...
    if targets.dim() == 2:
        # batch x len
        scores = -logits.gather(2, targets.unsqueeze(-1)).squeeze(-1)
    else:
        scores = -(logits * targets).sum(-1)

    scores = scores * (1 - label_smooth) - logits.mean(2) * label_smooth
    if sentence_wise:
        return (scores * masks).sum(1) / masks.sum(1)
    return (scores * masks).sum() / masks.sum()
...
def stratified_resample(weights, W):
    positions = (
        weights.new_zeros(weights.size(0), W).uniform_() + torch.arange(
            W, device=weights.get_device())[None, :].float()) / W
    cumulative_weights = weights.cumsum(1)
    difference_selects = (positions[:, :, None] <
...
def stratified_resample_noduplicated(weights, W0):
    ...
    b = ((outs[:, None, :] - outs[:, :, None] == 0
          ).float().cumsum(2).cumsum(2) == 1).float().sum(1)
...
def topK_search(logits, mask_src, N=100):
    ...
    def span(ins):
        ...
        return ins[:, :, None].expand(ins.size(0), ins.size(1),
                                      ins.size(1)) + inds[None, :, :]
    ...
    for k in range(1, N):
        ...
        S_t[:, 1:] += (
            (S_t[:, 1:] - S_t[:, :-1]) == 0).float() * INF  # remove duplicates
        S_t, _inx2 = S_t.sort(1)
        I_t = torch.cat([heap_inx[:, :, k:], I_t], 2).gather(
            2,
            _inx.gather(1, _inx2)[:, None, :].expand(batch_size, src_len,
                                                     _inx.size(-1)))
    ...
    output = output.transpose(2, 1).contiguous().view(batch_size * N,
                                                      src_len)  # (B x N) x Ts
    output = Variable(output)
    mask_src = mask_src[:, None, :].expand(batch_size, N,
                                           src_len).contiguous().view(
                                               batch_size * N, src_len)
...
class HighwayBlock(nn.Module):
    ...
    def forward(self, *x):
        ...
        return self.layernorm(x[0] * g +
                              self.dropout(self.layer(*x)) * (1 - g))
...
class Attention(nn.Module):
    ...
    def forward(self,
        ...
        if self.relative_pos:
            ...
            if relation is None:
                relation = query.new_ones(key.size(1),
                                          key.size(1)).triu(1).long()
                relation = relation - relation.t() + 1

                relation = relation.expand(
                    key.size(0), key.size(1), key.size(1))
            ...
            # batch x trg_len x trg_len
            pos_products = torch.gather(pos_products, 2, relation)
    ...
    def sent_cost(self,
        ...
        logits = self(queries, keys, value=None, mask=masks, relation=None)
        loss[name] = cross_entropy_matrix(
            logits,
            targets,
            masks,
            label_smooth=label_smooth,
            sentence_wise=sentence_wise)
        return loss
...
class PointerHead(nn.Module):
    ...
    def forward(self, query, key, mask=None, relation=None):
        ...
        logits = self.attention(query, key, None, mask,
                                relation)  # (B x n) x T x (D/n)
        return logits
...
class MultiHead2(nn.Module):
    ...
    def forward(self, query, key, value, mask=None, relation=None):
        query, key, value = self.wq(query), self.wk(key), self.wv(
            value)  # B x T x D
        B, Tq, D = query.size()
        _, Tk, _ = key.size()
        N = self.n_heads
        ...
        query, key, value = (x.contiguous().view(B, -1, N, D // N).transpose(
            2, 1).contiguous().view(B * N, -1, D // N)
                             for x in (query, key, value))

        if mask is not None:
            if mask.dim() == 2:
                mask = mask[:, None, :].expand(B, N, Tk).contiguous().view(
                    B * N, -1)
            else:
                mask = mask[:, None, :, :].expand(B, N, Tq,
                                                  Tk).contiguous().view(
                                                      B * N, Tq, Tk)

        if relation is not None:
            assert (relation.dim() == 3)
            relation = relation[:, None, :, :].expand(B, N, Tq,
                                                      Tk).contiguous().view(
                                                          B * N, Tq, Tk)

        outputs = self.attention(query, key, value, mask,
                                 relation)  # (B x n) x T x (D/n)
        outputs = outputs.contiguous().view(B, N, -1, D // N).transpose(
            2, 1).contiguous().view(B, -1, D)
...
class FeedForward(nn.Module):
    ...
    def forward(self, x):
        return self.linear2(self.dropout(F.relu(
            self.linear1(x))))  # adding dropout in feedforward layer
    ...
    def __init__(self,
        ...
        if cross:
            self.crossattn = ResidualBlock(
                MultiHead2(args.d_model, args.d_model, args.n_cross_heads,
                           args.attn_drop_ratio),
                args.d_model,
                args.drop_ratio,
                order=order)
    ...
    def __init__(self,
        ...
        self.layers = nn.ModuleList([
            Block(
                ...
                local=(i < local),
                relative_pos=relative_pos) for i in range(args.n_layers)
        ])
...
class IO(nn.Module):
    def __init__(self, field, args):
        ...
        self.field = field
        self.args = args
        self.out = nn.Linear(args.d_embedding, len(field.vocab), bias=False)
        self.scale = math.sqrt(args.d_embedding)

        # initialize the embedding weights using N(0, 1/sqrt(d_model))
        if not args.uniform_embedding_init:
            nn.init.normal_(
                self.out.weight, mean=0, std=args.d_embedding**-0.5)
    ...
    def cost(self,
        ...
        loss[name] = cross_entropy_with_smooth(
            self.o(outputs), targets, label_smooth, reweight=weights)
        return loss
...
class CopyIO(IO):
    ...
    def __init__(self, field, args):
        ...
        self.ptr = PointerHead(args.d_embedding, drop_ratio=0)
        self.gate = nn.Linear(args.d_embedding)
...
class MulIO(IO):
    ...
    def __init__(self, field, args):
        ...
        self.dyn = args.dyn
        self.rounter = nn.Linear(args.d_embedding,
                                 args.d_embedding * self.width)
        self.predictor = FeedForward(
            args.d_embedding, args.d_hidden, d_output=2)
    ...
    def cost(self, targets, masks, outputs, label_smooth=0.0, name=None):
        ...
        shifted_targets, shifted_masks = shift(targets, self.width), shift(
            masks, self.width)
        block_outputs = self.expand(outputs).transpose(
            3, 2)  # batch_size x seq-size x block-size x d_model

        if self.dyn == 0:
            shifted_targets, block_outputs = with_mask(shifted_targets,
                                                       block_outputs,
                                                       shifted_masks.byte())
            loss[name] = cross_entropy_with_smooth(
                self.out(block_outputs), shifted_targets, label_smooth)
            ...
            with torch.no_grad():
                scores = log_softmax(self.out(block_outputs)).gather(
                    -1, shifted_targets.unsqueeze(-1)).squeeze(
                        -1)  # batch_size x seq-size x block-size
                acceptance, new_masks = self.viterbi(
                    scores,
                    shifted_masks,
                    self.args.constant_penalty,
                    random=False)
                _, new_random_masks = self.viterbi(
                    scores,
                    shifted_masks,
                    self.args.constant_penalty,
                    random=True)
            ...
            if self.printer_param[1] % self.printer_param[0] == 1:
                print(
                    'rank{} sample:\t'.format(self.args.local_rank),
                    colored_seq(
                        self.field.reverse(targets)[0],
                        acceptance[0, :].cpu().tolist()))

            # use another predictor to predict the beam-searched sequence!
            predictions = self.predict(outputs)
            acceptance, predictions = with_mask(acceptance, predictions,
                                                masks.byte())
            loss['ACC'] = F.cross_entropy(predictions, acceptance)
            loss['#SPEEDUP'] = 1 / (1 - acceptance.float().mean() + 1e-9)

            optima_targets, optima_block_outputs = with_mask(
                shifted_targets, block_outputs, new_masks.byte())
            random_targets, random_block_outputs = with_mask(
                shifted_targets, block_outputs, new_random_masks.byte())
    ...
    def viterbi(self, scores, shifted_masks, c=0, random=False):
        ...
        scores = shift(
            scores * shifted_masks, blocksize, right=True,
            value=-INF)  # right-shifting

        decisions = scores.new_zeros(batchsize, seqsize,
                                     blocksize).long()  # all starts from 0
        ...
        for t in range(1, seqsize):
            ...
            outputs[:, t, 0] = max_outputs + scores[:, t,
                                                    0]  # best score for reject
            outputs[:, t,
                    1:] = outputs[:, t - 1, :
                                  -1] + scores[:, t,
                                               1:]  # best score for accept 1,2,3,...

            reject_decisions = decisions[:, :t].gather(
                2, max_indx[:, None, None].expand(batchsize, t,
                                                  1))  # best reject decision
        ...
        best_decision = decisions.gather(
            2, best_indx[:, None, None].expand(batchsize, seqsize, 1))

        acceptance = (best_decision.squeeze(-1) != 0).long()
        new_masks = scores.new_zeros(batchsize, seqsize, blocksize).scatter_(
            2, best_decision, 1)
    ...
    def search(self, scores, K=8):
        batchsize, seqsize, blocksize = scores.size()
        scores = shift(
            scores, blocksize, right=True, value=-INF)  # right-shifting
        scores = torch.cat(
            [scores, scores.new_zeros(batchsize, seqsize, 1) - INF],
            dim=2)  # safely guard

        decisions = scores.new_zeros(batchsize, seqsize,
                                     K).long()  # all starts from 0
        outputs = scores[:, 0, :].gather(1, decisions[:, 0, :])
        outputs[:, 1:].add_(-INF)

        for t in range(1, seqsize):
            reject_outputs = outputs + scores[:, t, 0:1]
            accept_outputs = outputs + scores[:, t, :].gather(
                1, decisions[:, t - 1, :] + 1)
            ...
            decisions = decisions.gather(
                2, sorted_ind[:, None, :].expand(batchsize, seqsize, K))

        best_decision = decisions[:, :, 0]
        acceptance = (best_decision != 0).long()
        new_mask = scores.new_zeros(batchsize, seqsize, blocksize).scatter_(
            2, best_decision.unsqueeze(-1), 1)
...
class Seq2Seq(nn.Module):
    ...
    def trainable_parameters(self):
        all_param = Seq2Seq.get_params([self])
        return [all_param]
    ...
    def prepare_masks(self, inputs):
        ...
        if text.ndimension() == 2:  # index inputs
            masks = (text.data !=
                     self.fields[field].vocab.stoi['<pad>']).float()
        else:  # one-hot vector inputs
            masks = (text.data[:, :, self.fields[field].vocab.stoi['<pad>']] !=
                     1).float()
        return masks
    ...
    def prepare_data(self, batch, dataflow=['src', 'trg'], noise=None):
        # get the data
        output = []
        for i, v in enumerate(dataflow):
            if v is None:
                output += [None, None, None, None]
            else:
                output += list(self.prepare_field(batch, v))
        return tuple(output)

    def merge_losses(self, info, losses):
        for loss in losses:
            for w in loss:
                info['L@' + w] = loss[w]
                if w[0] != '#':
